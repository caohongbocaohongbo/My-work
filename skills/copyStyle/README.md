...
源目录就位于 `~/.claude/skills/copyStyle/`，会通过 `sync-to-github.sh` 同步到 `~/Documents/My-work-repo/skills/copyStyle/`。

如需软链到其他 AI CLI 的 skills 目录：

```
ln -s ~/.claude/skills/copyStyle "${CODEX_HOME:-$HOME/.codex}/skills/copyStyle"
```
