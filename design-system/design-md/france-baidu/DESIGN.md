...
| 页面 | Figma 节点 | 用途 |
|------|------------|------|
| Dashboard 总览 | `140:2` / `Screen_Dashboard` | 定义 4 指标卡、最近推荐表格、今日涨停板、涨停池趋势图的首页信息密度。 |
| 监控列表 | `144:2` / `144:3 Screen_监控列表` | 当前文档的主参考页面，用于定义应用框架、侧栏、筛选栏、Tab、监控表格、分页和状态标签。 |
| 推荐结果 | `150:2` / `Screen` | 定义推荐等级筛选、统计卡、WATCH LIST 结果卡、审核降级提示和价格趋势 mini chart。 |
| 手动筛选 | `142:2` / `Screen_手动筛选` | 定义左侧筛选条件面板、右侧筛选结果面板、空态插图和底部默认参数。 |
| 股票详情弹窗 | `141:2` | 定义 `1100px` 宽分析弹窗、Header 元数据和 10 个图表卡片。 |
...
Dashboard 对应 `140:2` 和截图 `france.jpg`。它是交易日进入系统后的第一屏，要求用户在 1080px 高度内同时看到核心指标、最近推荐和市场趋势，不允许做成营销型大 Hero。

### 指标卡

- 指标卡为 4 列等宽栅格，间距 `16px`，单卡高度约 `110px`，圆角 `12px`，白底或极浅灰底，边框 `#e0e3ea`。
- 文案结构固定为：标签 `14px / 400` 弱文本，数值 `28px / 600` 等宽数字。
- 四项顺序固定：监控股票数、今日新增涨停、今日推荐、上证指数。
- 上证指数按 A 股涨跌色显示：涨红、跌绿；其余指标默认正文色，今日新增涨停可以在业务需要时使用红色强调。

### 最近推荐面板

- 面板位于指标卡下方，高度约 `353px`。标题为“最近推荐”，右侧为“查看全部 →”文字链接。
- 推荐表格只展示最近 3 条，列为：代码、名称、评分、回撤、推荐等级、涨停日期。
- 推荐等级使用小标签，不使用大面积色块。`WATCH` 使用蓝色弱底，`BUY` 使用绿色弱底，`STRONG_BUY` 使用红/橙弱底。
- 面板底部必须保留审核汇总行：`审核后: STRONG_BUY=0 · BUY=0 · WATCH=3 | 审核通过率 60%`。数据可来自接口，缺失时也要保留结构。

### 市场总览

- 市场总览为左右两列：左侧“今日涨停板”约占 `40%`，右侧“涨停池概览趋势”约占 `60%`。
- 今日涨停板卡片需要突出大数字，例如 `46只`，字号约 `48px / 600`，使用涨色红。
- 板块分布使用细胶囊进度条，条高约 `10px`，颜色从百度蓝到青色；不要使用厚重彩色卡片。
- 趋势图为 ECharts，红/绿窄柱表示涨跌分布，蓝色线表示市场回撤或主趋势。图表坐标标签可弱化，重点是走势节奏和红绿蓝色板。
...
推荐结果页对应 `150:2` 和截图 `Screen.png`。它不是普通表格页，应以“筛选 + 统计 + 推荐卡片 + 价格趋势图”组织，方便用户判断审核降级原因。

### 顶部筛选

- 筛选栏高度约 `33px`，包含筛选日期、推荐等级下拉和查询按钮。
- 日期与等级控件宽度均为 `240px`，查询按钮为 `68 × 33px` 深色主按钮。
- 顶部右侧可展示“筛选日期: YYYY-MM-DD | 审核通过率: 60%”，但不能挤压主筛选控件。

### 等级统计卡

- 统计区为 4 列，间距 `24px`，单卡高度 `116px`。
- 卡片顺序：`STRONG_BUY`、`BUY`、`WATCH`、`总计评分`。
- `WATCH` 激活卡使用白底 + 边框；非激活统计卡使用 `#f7f8fa` 浅灰底。
- 数值色彩：`STRONG_BUY` 红、`BUY` 橙、`WATCH` 蓝、总计正文色。

### 推荐结果卡片

- 列表标题使用 `WATCH LIST` 这类大写等级标题，右侧胶囊显示结果数量。
- 每只股票是一张浅灰结果卡，最小高度约 `403px`，圆角 `12px`。
- 卡片头部必须包含排名、股票名称、代码、评分、回撤、涨停日、参考价、现价。
- 审核降级提示使用红色文本，格式示例：`审核降级: BUY→WATCH · 换手充分，强势封板`。
- 因子标签使用浅灰底小 chip，例如“回撤幅度 10/10”“量能趋势 7/10”“均线多头 7/10”“强势确认 6/10”“尾盘买点 4/10”。
- 右上角操作按钮为蓝色小按钮，文案可为 `WATCH 观察`，按钮仍需调用真实审核/批准 API。

### 推荐卡片图表

- 每个推荐卡片必须包含“关注后交易日趋势图”，图表卡白底、细边框、`8px` 圆角。
- 图表标题为 `16px / 600`，副标题和图例为 `12px` 弱文本。
- 必须展示收盘价蓝线、回撤蓝紫线、参考价灰色虚线；无历史 K 线时允许用当前行情生成前端可视化兜底，但要继续请求真实详情接口补全。

## 手动筛选页

手动筛选页对应 `142:2` 和截图 `france.png`。页面采用左侧筛选面板 + 右侧结果面板，不使用横向铺满的普通表单。

### 左侧筛选条件

- 左侧筛选面板宽度 `340px`，与右侧结果面板间距 `20px`。
- 标题“筛选条件”使用 `20px / 600`。
- 核心范围字段依次为：回撤范围、量比范围、换手率范围、流通市值范围、PE 上限。
- 范围输入为双输入框 + 中间短横线结构，输入框高度 `36px`，背景 `#f7f8fa`，边框 `#e0e3ea`，圆角 `4px`。
- 高级字段可折叠或纵向放置：日期起止、板块、连板数、封板时间。
- 底部按钮固定在面板底部附近：`执行筛选` 为蓝色主按钮，高 `46px`；`重置条件` 为白底描边按钮；`导出CSV` 在无结果时禁用。

### 右侧筛选结果

- 结果面板为白底、`1px #e0e3ea` 边框、`12px` 圆角，最小高度约 `948px`。
- Header 左侧为“筛选结果”与数量胶囊，右侧为搜索框和查询按钮。
- 初始空态必须居中显示 SVG 放大镜图形，不得使用 emoji；文案为“设置筛选条件后点击「执行筛选」”“筛选结果将在此处展示”。
- 底部保留默认参数说明：`默认参数: 回撤3%-10% | 量比1-5 | 换手率5%-10% | 流通市值50-200亿 | PE≤50`。
- 有数据时在结果面板内显示表格，表格仍遵循监控列表的数字对齐和颜色规范。
...
- ECharts 图表容器使用白底、`8px` 到 `12px` 圆角、细边框和弱网格线。价格主线使用百度财经经典蓝 `#0065C8`，回撤线使用 `#4e6ef2`，参考价线使用 `rgba(0,0,0,0.25)` 虚线，成交量柱按涨跌色区分。
...
- 当 `/api/stocks/<code>` 响应慢或 `kline` 为空时，前端应先用表格行中的 `basic` 数据展示弹窗 Header 与图表骨架，并生成可读的兜底趋势图；接口返回后再用真实 K 线替换。禁止让弹窗长时间只显示整屏 Loading。
...
|------|------|
| 宽度 | `1100px` |
| 高度 | 基础态 `860px`；多图表分析态使用 `max-height: calc(100vh - 48px)` 的可滚动长面板，不应压缩图表高度 |
...
- 如果详情接口仍在同步，Header 下方可以显示 `12px` 弱文本状态“正在同步历史 K 线与图表数据…”，但图表卡片必须先展示。
- 所有价格、百分比、金额、PE、量比、换手率和时间类数字必须使用 tabular numbers，保证表格和图表标签对齐。

### 个股详情图表组件

所有图表卡片遵循同一结构：标题 `16px / 600`，副标题 `12px / 400`，图例为 `6px` 圆点 + `12px` 文本，图表区域背景透明，透出白色卡片底。坐标轴文字使用 `11px` 和弱文本色，网格线使用 `rgba(0,0,0,0.06)` dashed。

股票详情多图表布局：
- 第一张“价格走势”图横跨整行，高度约 `250px`，是弹窗主图。
- 其余图表使用双列栅格，每张卡片最小高度约 `269px`，图表区域约 `210px`。
- “涨停板时间轴”横跨整行，确保每个涨停日期能被看清。
- 弹窗打开后 1 秒内必须看到股票名称、元数据和 10 个图表容器；真实 K 线慢时展示同步提示，不阻塞页面。
