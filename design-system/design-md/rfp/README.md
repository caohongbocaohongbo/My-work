...
- **状态标签 (Status Tags)** — 6 种语义紧凑标签 (支付中/已失败/已支付/已取消/待支付/待填写), 4px 圆角无边框, Inter Bold 12px, 内边距 2px/4px, 支持单元格内叠加；另有 h24px 大胶囊变体 (未报价/签约中), PingFang SC Regular 14px
...
- **数据指标图标集 (Metric Icons)** — 32×32 圆形容器 (内含 20×20 图标, 6px 内边距), 20+ 业务指标语义 (报价/签约/中签/城市覆盖/采购/权益), 面形/线形混用, 含 24/28/30/31 非标准变体
...
15. **状态标签 6 种** — 支付中(蓝)/已失败(红)/已支付(绿)/已取消(灰)/待支付(橙)/待填写(青)，4px 圆角无边框紧凑型，内边距 2px/4px，可叠加使用；标题/流程场景另用 h24px 大胶囊变体 (未报价/签约中)
